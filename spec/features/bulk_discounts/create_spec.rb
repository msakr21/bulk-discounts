...
RSpec.describe 'create new merchant bulk discount' do
  ...
  it "updates all the bulk discount id of all invoice items that qualify for the discount and don't have another higher discount." do
    ...
    updated_rash_guard_invoice = InvoiceItem.find(@rash_guard_invoice_2.id)
    updated_wax_invoice_invoice = InvoiceItem.find(@wax_invoice_invoice_2.id)
    updated_snorkel_invoice = InvoiceItem.find(@snorkel_invoice.id)

    expect(updated_rash_guard_invoice.bulk_discount_id).to_not eq(@default_price_2.id)
    expect(updated_wax_invoice_invoice.bulk_discount_id).to_not eq(@default_price_2.id)
    expect(updated_snorkel_invoice.bulk_discount_id).to_not eq(@default_price_2.id)

    new_discount = BulkDiscount.find(updated_snorkel_invoice.bulk_discount_id)

    expect(updated_wax_invoice_invoice.bulk_discount_id).to eq(new_discount.id)
    expect(updated_snorkel_invoice.bulk_discount_id).to eq(new_discount.id)
    expect(new_discount.amount).to eq(10)
    expect(new_discount.threshold).to eq(10)

    visit new_merchant_bulk_discount_path(@surf_designs)

    fill_in "discount_amount", with: 15
    ...
    expect(updated_rash_guard_invoice.bulk_discount_id).to eq(new_discount.id)
    expect(updated_snorkel_invoice.bulk_discount_id).to eq(new_discount.id)
    expect(updated_wax_invoice_invoice.bulk_discount_id).to_not eq(new_discount.id)

    new_discount_2 = BulkDiscount.find(updated_wax_invoice_invoice.bulk_discount_id)

    expect(new_discount_2.amount).to eq(15)
    expect(new_discount_2.threshold).to eq(15)
  end
end